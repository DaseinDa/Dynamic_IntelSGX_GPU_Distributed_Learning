...
inerror = torch.nn.grad.conv2d_input(input.shape, weight, outerror,padding=1).cuda()
wd = torch.nn.grad.conv2d_weight(input, weight.shape, outerror,padding=1).cuda()
...
print("Time execution grad_conv on GPU is:{}".format((end-start)*1000))
print(wd.shape)
