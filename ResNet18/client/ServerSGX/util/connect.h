...
template<typename T>
void sendVector(size_t player, const std::vector<T> &vec) {
        std::cout<<"The message size is:"<<vec.size() * sizeof(T)<<std::endl;
...
template<typename T>
void receivedTest(size_t player, std::string *text) {
	if(!communicationReceivers[player]->receiveMsg(&text, 100, 0)) {
        std::cout << "Receive text error" << std::endl;
    }
}
