...
void initCommunication(std::string addr, int port, int player, int mode)
{
	...
	std::cout<<"initCommunication function"<<std::endl;

	if (mode == 0)//发送出去
	{	
		communicationSenders[player] = new BmrNet(temp, port);//发送给player
		communicationSenders[player]->connectNow();
	}
	else//收到
	{	
		std::cout<<"The receive port:"<<port<<"The receive address:"<<temp<<std::endl;
		communicationReceivers[player] = new BmrNet(port);//从player收到
...
void initializeCommunication(int* ports, int num_parties) {
	...
	for (i = 0; i < num_parties; i++)
	{
		if (i != partyNum)
		{
			threads[i * 2 + 1] = std::thread(initCommunication, addrs[i], ports[i * 2 + 1], i, 0);//发送出去
			threads[i * 2] = std::thread(initCommunication, "127.0.0.1", ports[i * 2], i, 1);//收到
		}
	}
...
void initializeCommunicationSerial(int *ports, int num_parties) { //Use this for many parties
	...
	for (int i = 0; i < num_parties; i++)
	{
		if (i<partyNum)
		{	
		  initCommunication( addrs[i], ports[i * 2 + 1], i, 0);//party和i交流
		  initCommunication("127.0.0.1", ports[i * 2], i, 1);//i和party交流
		}
		else if (i>partyNum)
		{
...
void initializeCommunication(char* filename, int party, int num_parties) {
	...
	for (int i = 0; i < num_parties; i++) {
		if (fgets(buff, STRING_BUFFER_SIZE, f)) {
            ...
            ports[2 * i] = 52000 + i*num_parties + partyNum;
            ports[2 * i + 1] = 52000 + partyNum*num_parties + i;
        } else {
...
void initializeCommunication(std::vector<std::string> &ips, int party, int num_parties) {
    ...
    for (int i = 0; i < num_parties; i++) {
        addrs[i] = ips[i];
        ports[2 * i] = 52000 + i * num_parties + party;//第i个party和party交流的端口
        ports[2 * i + 1] = 52000 + party * num_parties + i;//party和第i个party交流的端口
    }
