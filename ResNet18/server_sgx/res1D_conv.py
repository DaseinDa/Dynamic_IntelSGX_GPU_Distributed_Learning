...
conv = torch.nn.functional.conv2d(data_npy,weight_npy,bias_npy,stride=1,padding=1)
conv+=torch.nn.functional.conv2d(res_npy,res_weight_npy,bias_npy,stride=2,padding=0)
torch.cuda.synchronize()
end = time.time()

print("Time execution res1D conv on GPU is:{}".format((end-start)*1000))
