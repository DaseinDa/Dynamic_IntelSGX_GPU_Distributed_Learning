...
inerror = torch.nn.grad.conv2d_input(input.shape, weight,outerror,stride=1).float()
wd = torch.nn.grad.conv2d_weight(input, weight.shape, outerror,padding=0)
...
print("Time execution grad conv padding0 on GPU is:{}".format((end-start)*1000))
